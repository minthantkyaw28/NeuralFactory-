...
const FactoryWalls: React.FC = () => {
  ...
  return (
    <group>
      ...
      {/* Wall Panel Details */}
      <Grid position={[0, 2500, -2450]} args={[9000, 5000]} rotation={[Math.PI/2, 0, 0]} cellSize={100} cellColor="#4b5563" sectionColor="#6b7280" infiniteGrid={false} />
    </group>
  );
};
...
const StationGantry: React.FC<{ active: boolean }> = ({ active }) => {
    ...
    return (
        <group position={[0, 0, 0]}>
            <mesh position={[-7, 7, 0]}><boxGeometry args={[1.5, 14, 1.5]} /><meshStandardMaterial color="#eab308" /></mesh>
            <mesh position={[7, 7, 0]}><boxGeometry args={[1.5, 14, 1.5]} /><meshStandardMaterial color="#eab308" /></mesh>
            <mesh position={[0, 13, 0]}><boxGeometry args={[16, 1.5, 1.5]} /><meshStandardMaterial color="#020617" /></mesh>
            <group ref={gantryRef} position={[0, 6, 0]}>
                <mesh position={[0, 3, 0]}><boxGeometry args={[3, 3, 3]} /><meshStandardMaterial color="#f97316" /></mesh>
                <mesh position={[0, 0, 0]}><cylinderGeometry args={[0.3, 0.3, 6]} /><meshStandardMaterial color="#94a3b8" /></mesh>
            </group>
        </group>
    )
}
...
const StationWelder: React.FC<{ active: boolean }> = ({ active }) => {
    ...
    return (
        <group position={[0, 0, -4]}>
            <mesh position={[0, 3, 0]}><cylinderGeometry args={[2, 3, 6, 32]} /><meshStandardMaterial color="#b91c1c" /></mesh>
            <group position={[0, 6, 0]} ref={group}>
                <mesh position={[0, 0, 0]}><boxGeometry args={[10, 1.5, 1.5]} /><meshStandardMaterial color="#f97316" /></mesh>
                <mesh position={[4, -3, 0]}><cylinderGeometry args={[0.3, 0.3, 6]} /><meshStandardMaterial color="#94a3b8" /></mesh>
                <mesh position={[-4, -3, 0]}><cylinderGeometry args={[0.3, 0.3, 6]} /><meshStandardMaterial color="#94a3b8" /></mesh>
            </group>
        </group>
    )
...
const StationChamber: React.FC<{ active: boolean }> = ({ active }) => (
    <group position={[0, 0, -4]}>
        <mesh position={[0, 5, 0]} rotation={[0, 0, Math.PI/2]}>
            ...
            <meshStandardMaterial color="#d97706" metalness={0.7} roughness={0.2} side={THREE.DoubleSide} />
        </mesh>
        <mesh position={[0, 5, 0]} rotation={[0, 0, Math.PI/2]}>
             <cylinderGeometry args={[4.2, 4.2, 13.8, 32]} />
             <meshStandardMaterial color="#0f172a" /> 
        </mesh>
        <mesh position={[-6, 1, 0]}><boxGeometry args={[2, 2, 8]} /><meshStandardMaterial color="#374151" /></mesh>
        <mesh position={[6, 1, 0]}><boxGeometry args={[2, 2, 8]} /><meshStandardMaterial color="#374151" /></mesh>
        {active && <pointLight position={[0, 5, 0]} color="orange" intensity={15} distance={25} />}
    </group>
);
...
const StationScanner: React.FC<{ active: boolean }> = ({ active }) => {
    ...
    return (
        <group position={[0, 0, -4]}>
            <mesh position={[0, 5, 0]}><boxGeometry args={[2, 10, 2]} /><meshStandardMaterial color="#0f766e" /></mesh>
            <group position={[0, 8, 0]} ref={ref}>
                ...
                <mesh position={[3, 0, 0]}><boxGeometry args={[2, 3, 2]} /><meshStandardMaterial color="#10b981" /></mesh>
                {active && <mesh position={[3, -4, 0]}><coneGeometry args={[2, 8, 32]} /><meshStandardMaterial color="#10b981" transparent opacity={0.2} /></mesh>}
            </group>
        </group>
    )
}
...
const ShippingDock: React.FC = () => (
    <group position={[0, 0, -2]}>
        <mesh position={[3, 3, -2]}><boxGeometry args={[6, 6, 10]} /><meshStandardMaterial color="#991b1b" /></mesh>
        <mesh position={[-3, 3, -4]}><boxGeometry args={[6, 6, 6]} /><meshStandardMaterial color="#1e3a8a" /></mesh>
        <Text position={[0, 7, 2]} color="white" fontSize={1.5} rotation={[0, Math.PI, 0]}>LOGISTICS</Text>
    </group>
);
  ...
  return (
    <div className="w-full h-full bg-gray-900 rounded-lg overflow-hidden shadow-2xl border border-gray-700">
      ...
      <Canvas shadows camera={{ position: [0, 3500, 6000], fov: 60, far: 20000 }}>
        ...
        <fog attach="fog" args={['#1f2937', 3000, 12000]} />
        
        <Suspense fallback={null}>
          <Environment preset="warehouse" />
          
          <ambientLight intensity={3.0} color="#ffffff" />
          <hemisphereLight intensity={2.0} color="#ffffff" groundColor="#4b5563" />
          
          <directionalLight position={[500, 3000, 500]} intensity={5.0} castShadow shadow-mapSize={[2048, 2048]} shadow-bias={-0.0001} />

          {/* --- SPOTLIGHTS (Scaled Down) --- */}
          {/* Spacing lines 1500 apart */}
          <spotLight position={[0, 3000, -1500]} angle={1.0} penumbra={1} intensity={3000000} color="#ffffff" distance={8000} castShadow />
          <spotLight position={[0, 3000, 0]} angle={1.0} penumbra={1} intensity={3000000} color="#ffffff" distance={8000} castShadow />
          <spotLight position={[0, 3000, 1500]} angle={1.0} penumbra={1} intensity={3000000} color="#ffffff" distance={8000} castShadow />
          ...
          <group position={[0, -200, 0]}>
            ...
            <Grid position={[0, 0.05, 0]} args={[9000, 5000]} cellSize={200} cellColor="#4b5563" sectionColor="#6b7280" />

            {/* --- WALLS (Scaled Down) --- */}
            <FactoryWalls />

            {/* --- PRODUCTION LINES (Scale 20) --- */}
            {/* Spaced 1500 apart. */}
